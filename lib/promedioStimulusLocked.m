function tablaSEN=promedioStimulusLocked(name,pareja,canal)
...
for numpar=1:numeroparejas,
    ...
    for i=1:length(vec_estim),
        ...
        if (ok==1),
            inicio=CNT.event(vec_estim(i),3)-round(xmin*CNT.rate/1000)+1;
            final=CNT.event(vec_estim(i),3)+round(xmax*CNT.rate/1000);
